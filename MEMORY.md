...
* **Seamless Warp Triggers:** Position-based triggers (`mods/system_waypoints/connections.lua`) warp players seamlessly between levels (e.g., Bob-omb Battlefield → Whomp's Fortress) without Castle Hub, creating a contiguous MMORPG map layout.

## Implementation: PvP & Movement Mechanics

* **Knockback System:** Relies on flattened sync keys (`kb_immune` in `gPlayerSyncTable`, e.g., Warrior's Rage) and core engine flags (`MARIO_METAL_CAP`) for hit reactions. Immunity correctly nullifies velocity changes without desync across clients.
* **Glide/Fludd Hood:** Hook `HOOK_ON_SET_MARIO_ACTION` to clear active states (e.g., `is_gliding = false`) when Mario enters airborne knockback actions (`ACT_BACKWARD_AIR_KB`, `ACT_FORWARD_AIR_KB`) to prevent floating physics desyncs.
